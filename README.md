...
- **🔄 后台进程生命周期管理**：Rust 后端负责本地工具进程的启动、监控和清理，避免孤儿后台进程占用资源。
...
- **🔄 Auto Process Lifecycle Control**: Rust starts, monitors, and cleans up local tool processes so background work does not linger.
...
pnpm tauri dev
```
*This starts the SvelteKit dev server, compiles the Rust backend, and opens the window.*
...
1. Build the Tauri app:
   ...
   pnpm release:build
   ```
`release:build` automatically prepares the bundled Node and FFmpeg runtimes, verifies `yt-dlp`, runs validation, and then builds the app.
