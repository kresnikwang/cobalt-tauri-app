...
const ytdlpCookiesPath = process.env.YTDLP_COOKIES || "/home/ubuntu/cobalt-api/youtube-cookies.txt";
...
const createYtdlpTunnelResponse = (url) => {
    ...
    return {
        ...
        body: {
            status: "tunnel",
            url: tunnel.toString(),
            filename: youtubeFilename(url)
        }
    }
}

const ytdlpErrorMessage = (stderr) => {
    ...
    if (
        ...
        return {
            status: 401,
            message: "YouTube login cookies expired. Please refresh the server YouTube cookies."
        };
    }

    return {
        status: 502,
        message: "YouTube download failed on the media server."
    };
}
...
export const runAPI = async (express, app, __dirname, isPrimary = true) => {
    ...
    app.post('/', async (req, res) => {
        ...
        if (parsed.host === "youtube") {
            const result = createYtdlpTunnelResponse(normalizedRequest.url);
    ...
    app.get('/yt-dlp', apiTunnelLimiter, async (req, res) => {
        ...
        child.on("close", (code) => {
            ...
            if (!started && !res.headersSent) {
                const error = ytdlpErrorMessage(stderr);
