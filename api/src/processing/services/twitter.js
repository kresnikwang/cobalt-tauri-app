...
function getVariantHeight(variant) {
    const match = variant.url?.match(/\/(\d+)x(\d+)\//);
    return match ? Number(match[2]) : 0;
}

function bestQuality(arr, quality = "max") {
    const targetHeight = quality === "max" ? Infinity : Number(quality) || Infinity;
    const videos = arr.filter(v => v.content_type === "video/mp4");
    const candidates = videos.filter(v => {
        const height = getVariantHeight(v);
        return !height || height <= targetHeight;
    });
    const pool = candidates.length ? candidates : videos;
    const best = pool.reduce((a, b) => Number(a?.bitrate) > Number(b?.bitrate) ? a : b);

    return stripVideoURL(best?.url);
}
...
export default async function({ id, index, toGif, dispatcher, alwaysProxy, quality, subtitleLang }) {
